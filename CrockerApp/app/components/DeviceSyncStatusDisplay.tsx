...
export const DeviceSyncStatusDisplay: React.FC<DeviceSyncStatusDisplayProps> = ({
  showDetails = true,
  onSyncDevice,
}) => {
  const { 
  ...
  const handleSyncDevice = async (deviceStatus: DeviceSyncStatus) => {
    try {
      const success = await syncDeviceEvents(deviceStatus.kidId, deviceStatus.deviceId);
  ...
  const handleMarkAllForResync = async () => {
    try {
      ...
      Alert.alert(
        'Error',
        error instanceof Error ? error.message : 'Failed to mark devices for resync'
      );
    }
  };
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                'Error',
                error instanceof Error ? error.message : 'Failed to sync all devices'
              );
            }
          },
        },
              ...
              {showDetails && (
                <View style={{ marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: '#eee' }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <View>
                      <Text style={{ fontSize: 12, color: '#999' }}>Last Sync</Text>
                      <Text style={{ fontSize: 14, color: '#333' }}>
                        {formatLastSync(deviceStatus.lastSuccessfulSync)}
                      </Text>
                    </View>
                    <View>
                      <Text style={{ fontSize: 12, color: '#999' }}>Events Synced</Text>
                      <Text style={{ fontSize: 14, color: '#333' }}>
                        {deviceStatus.syncedEventCount}
                      </Text>
                    </View>
                    <View>
                      <Text style={{ fontSize: 12, color: '#999' }}>Retries</Text>
                      <Text style={{ fontSize: 14, color: '#333' }}>
                  ...
                  {deviceStatus.failureReason && (
                    <View style={{ marginTop: 8 }}>
                      <Text style={{ fontSize: 12, color: '#999' }}>Last Error</Text>
                  ...
                  {deviceStatus.nextRetryAt && (
                    <View style={{ marginTop: 4 }}>
                      <Text style={{ fontSize: 12, color: '#999' }}>Next Retry</Text>
