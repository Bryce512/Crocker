...
export const useEventSync = (): UseEventSyncReturn => {
   ...
   */
  const syncDeviceEvents = useCallback(
    async (kidId: string, deviceId?: string): Promise<boolean> => {
      try {
        setIsSyncing(true);
        console.log(
          `🔄 Syncing events for kid ${kidId}${
            deviceId ? ` on device ${deviceId}` : ""
          }`
        );

        const success = await eventSyncService.syncDeviceEvents(
          kidId,
          deviceId
        );
   ...
   */
  const getSyncStatusForDevice = useCallback(
    async (deviceId: string, kidId: string): Promise<DeviceSyncStatus> => {
      try {
        const status = await eventSyncService.getSyncStatusForDevice(
          deviceId,
          kidId
        );
        ...
        setAllSyncStatus((prev) => {
          const filtered = prev.filter(
            (s) => !(s.deviceId === deviceId && s.kidId === kidId)
          );
  ...
  const devicesNeedingSync = allSyncStatus.filter((status) => {
    ...
    if (now.getTime() - status.lastSuccessfulSync.getTime() > syncIntervalMs)
      return true;
