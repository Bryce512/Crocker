...
class EventSyncService {
  ...
  private isSyncNeeded(syncStatus: DeviceSyncStatus): boolean {
    ...
    if (
      now.getTime() - syncStatus.lastSuccessfulSync.getTime() >
      syncIntervalMs
    ) {
  ...
  private async scheduleRetry(deviceId: string, kidId: string): Promise<void> {
    ...
    console.log(
      `⏰ Retry scheduled for device ${deviceId} at ${syncStatus.nextRetryAt.toISOString()}`
    );
  }
      ...
      if (stored) {
        ...
        const status: DeviceSyncStatus = {
          ...
          lastSuccessfulSync: parsed.lastSuccessfulSync
            ? new Date(parsed.lastSuccessfulSync)
            : null,
  ...
  private async loadSyncStatusFromStorage(): Promise<void> {
    try {
      ...
      console.log(
        `📂 Loading ${syncKeys.length} sync status records from storage`
      );
      ...
      for (const key of syncKeys) {
        try {
          ...
          console.error(
            `Failed to parse sync status for key ${key}:`,
            parseError
          );
        }
      }
  ...
  private async checkPendingSyncs(): Promise<void> {
    try {
      ...
      console.log(
        `🔍 Found ${dueRetries.length} due retries out of ${retryQueue.length} queued`
      );

      for (const retry of dueRetries) {
        console.log(
          `🔄 Processing retry for device ${retry.deviceId}, kid ${retry.kidId}`
        );
    ...
    try {
      ...
      for (const kid of devicesWithKids) {
        if (kid.deviceId) {
          ...
          if (this.isSyncNeeded(syncStatus)) {
            console.log(
              `📅 Periodic sync needed for device ${kid.deviceId}, kid ${kid.id}`
            );
  ...
  private async removeFromRetryQueue(item: RetryQueueItem): Promise<void> {
    try {
      ...
      const filtered = queue.filter(
        (queueItem) =>
          !(
            queueItem.deviceId === item.deviceId &&
            queueItem.kidId === item.kidId
          )
      );
  ...
  public async getSyncStatusForDevice(
    deviceId: string,
    kidId: string
  ): Promise<DeviceSyncStatus> {
